...
#[derive(Clone)]
pub struct DeleteVoiceV1VoicesVoiceIdDeleteRequest<'a> {
    pub(crate) http_client: &'a ElevenLabsClient,
    pub voice_id: String,
    pub xi_api_key: Option<String>,
}
impl<'a> DeleteVoiceV1VoicesVoiceIdDeleteRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<serde_json::Value> {
        let mut r = self
            .http_client
            .client
            .delete(&format!("/v1/voices/{voice_id}", voice_id = self.voice_id));
...
impl<'a> ::std::future::IntoFuture for DeleteVoiceV1VoicesVoiceIdDeleteRequest<'a> {
